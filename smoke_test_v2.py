...
Uses synchronous TestClient for all requests.
"""
...
def get_db_pool_sync():
    ...
    # Get the existing event loop (created by TestClient) instead of creating a new one
    try:
    ...
    pool = loop.run_until_complete(asyncpg.create_pool(dsn, min_size=1, max_size=10, init=init_conn))
    return pool, loop


def query_user_viewings_sync(pool, loop, user_name: str) -> Optional[dict]:
...
def cleanup_user_record_sync(pool, loop, user_name: str) -> None:
...
def close_pool_sync(pool, loop) -> None:
    """Close the pool and event loop."""
    async def _close():
        await pool.close()
...
def run_smoke_test():
    ...
    try:
        # Step 1: Create test client (which triggers startup events)
        print("\n[1] Creating FastAPI test client...")
        client = TestClient(app)
        results["checks"].append(("FastAPI startup events triggered", "PASS", None))
        print("    ✓ Database initialized via startup events")
        
        # Step 2: Verify PIN endpoint works
        ...
        # Step 3: Get DB pool for direct queries (separate from app's pool)
        print("\n[3] Getting database connection pool for queries...")
        db_pool, db_loop = get_db_pool_sync()
        ...
        cleanup_user_record_sync(db_pool, db_loop, TEST_USER_NAME)
        ...
        # Step 5b: Query and verify unblock event was logged
        print("\n[5b] Querying PostgreSQL for unblock event...")
        import time
        time.sleep(0.5)  # Let the transaction settle
        viewings = query_user_viewings_sync(db_pool, db_loop, TEST_USER_NAME)
        ...
        # Step 6b: Query and verify the unblock event was closed
        ...
        # Step 7b: Query and verify temp unblock event was logged
        ...
        close_pool_sync(db_pool, db_loop)
        print(f"    ✓ Pool closed")
